...
export function NewResearchModal({ buttonProps }: NewResearchModalProps) {
  ...
  const handleSelection = useCallback(
    (type: "chat" | "playbook") => {
      ...
      if (type === "chat") {
        router.push(
          `/chat/new?context=${encodeURIComponent(JSON.stringify(context))}`,
        );
      } else {
        router.push(
          `/playbooks?context=${encodeURIComponent(JSON.stringify(context))}`,
        );
      }
    },
    [context, router],
  ...
  return (
    <>
      <Button onClick={handleNewResearchClick} {...buttonProps}>
